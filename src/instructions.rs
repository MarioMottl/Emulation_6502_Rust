...
impl AddressingMode {
    ...
    pub fn process(self, cpu: &CPU, arr: &[u8]) -> OperationInput {
        ...
        match self {
            ...
            AddressingMode::Relative => {
                ...
                OperationInput::UseRelative(rel)
            }
            AddressingMode::Absolute => {
                // Use [u8, ..2] from instruction as address
                // (Output: a 16-bit address)
                OperationInput::UseAddress(arr_to_addr(arr))
            }
            AddressingMode::AbsoluteX => {
                // Use [u8, ..2] from instruction as address, add X
                // (Output: a 16-bit address)
                OperationInput::UseAddress(arr_to_addr(arr).wrapping_add(extend_u8_to_u16(x)))
            }
            AddressingMode::AbsoluteY => {
                // Use [u8, ..2] from instruction as address, add Y
                // (Output: a 16-bit address)
                OperationInput::UseAddress(arr_to_addr(arr).wrapping_add(extend_u8_to_u16(y)))
            }
            AddressingMode::Indirect => {
                ...
                OperationInput::UseAddress(arr_to_addr(slice))
            }
            ...
            AddressingMode::IndirectIndexedY => {
                ...
                let start = arr[0];
                let slice = memory.get_slice(u16::from(start), 2);
                OperationInput::UseAddress(arr_to_addr(slice).wrapping_add(extend_u8_to_u16(y)))
            }
        }
    }
}

pub type DecodedInstr = (Instruction, OperationInput);
...
mod tests {
    ...
    fn zeropage_wrap_around() {
        ...
        use crate::instructions::OperationInput;
        ...
        assert!(matches!(
            ...
            OperationInput::UseAddress(19)
        ));
        assert!(matches!(
            AddressingMode::ZeroPageX.process(&cpu, &[250]),
            OperationInput::UseAddress(3)
        ));
    }
}